...
function ImgAndText(props) {
    ...
    return (
        <section ref={sectionRef} className={"relative" + ` ${props.style}`}>
            ...
            <span className="px-[2%] h-[70vh] flex justify-around gap-7 items-center overflow-x-hidden">
                {props.isImageLeft ? 
                <><img src={props.image} ref={imgRef} className="rounded-lg border-4 border-slate-50 drop-shadow-xl w-5/6" alt="placeholder"/>
                <h2 ref={textRef} className="text-slate-[#2C3233] text-4xl font-semibold text-justify w-full">{props.text}</h2></>
                :
                <><h2 ref={textRef} className="text-slate-[#2C3233] text-4xl font-semibold text-justify w-full">{props.text}</h2>
                <img src={props.image} ref={imgRef} className="rounded-lg border-4 border-slate-50 w-5/6 drop-shadow-xl" alt="placeholder"/></>}
            </span>
        </section>
    )
}
